...
class RobertaSelfAttention_matchKV(nn.Module):
    def __init__(self, args):
        ...
        if self.attn_map in ["IC2", "IC3", "IC5", "IC6"]:
    ...
    def forward(self, hidden_states, attention_mask=None, head_mask=None, encoder_hidden_states=None, encoder_attention_mask=None, past_key_value=None, output_attentions=False):
        ...
        elif self.attn_map in ["IC4", "IC5", "IC7", "IC8"]:

            if self.attn_map =="IC4":
                K1 = self.transpose_for_scores(self.act(self.K1(hidden_states)))
                V1 = self.transpose_for_scores(self.act(self.V1(hidden_states)))
                bs, length, n_head, hd = K1.shape
                forward_activity = torch.einsum('blhn,hn->blh', K1, self.ReadingHead)
                forward_events = backward_events = self._softmax(forward_activity) > (1.5 / length)
                V1 = V1 * self.sigmoid(forward_activity).view(bs, length , n_head, 1)

            elif self.attn_map =="IC7":
                K1 = self.transpose_for_scores(self.act(self.K1(hidden_states)))
                V1 = self.transpose_for_scores(self.act(self.V1(hidden_states)))
                bs, length, n_head, hd = K1.shape
                forward_activity = torch.einsum('blhn,hn->blh', K1, self.ReadingHead)
                forward_events  = self._softmax(forward_activity) > (2 / length)
                backward_events = self._softmax(forward_activity) > (1.5 / length)
                V1 = V1 * self.sigmoid(forward_activity).view(bs, length , n_head, 1)

            elif self.attn_map =="IC8":
                K1 = self.transpose_for_scores(self.act(self.K1(hidden_states)))
                V1 = self.transpose_for_scores(self.act(self.V1(hidden_states)))
                bs, length, n_head, hd = K1.shape
                forward_activity = torch.einsum('blhn,hn->blh', K1, self.ReadingHead)
                forward_events  = self._softmax(forward_activity) > (4 / length)
                backward_events = self._softmax(forward_activity) > (2 / length)
                V1 = V1 * self.sigmoid(forward_activity).view(bs, length , n_head, 1)

            elif self.attn_map =="IC5":
                K1 = self.transpose_for_scores(self.act(self.K1(hidden_states)))
                K2 = self.transpose_for_scores(self.act(self.K2(hidden_states)))
                V1 = self.transpose_for_scores(self.act(self.V1(hidden_states)))
                bs, length, n_head, hd = K1.shape
                forward_activity = torch.einsum('blhn,hn->blh', K1, self.ReadingHead)
                forward_events = self._softmax(forward_activity) > (1.5 / length)
                backward_activity = torch.einsum('blhn,hn->blh', K2, self.ReadingHead)
                backward_events = self._softmax(backward_activity) > (1.5 / length)
                V1 = V1 * (1 - (1-self.sigmoid(forward_activity).view(bs, length , n_head, 1)) *  (1-self.sigmoid(backward_activity).view(bs, length , n_head, 1)) )
